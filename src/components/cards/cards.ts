type ColumnType = 'pendente' | 'em-progresso' | 'finalizado';
...
class Card {
    ...
    updateButtons(): void {
        ...
        if (this.columnType === 'em-progresso') {
            ...
            this.addRightButton();
        } else if (this.columnType === 'finalizado') {
            this.addLeftButton();
        }
    }
    ...
    updateColumnType(columnId: string): void {
        this.columnType = columnId === "1" ? 'pendente' :
                          columnId === "2" ? 'em-progresso' : 'finalizado';
    }
}
    ...
    init(): void {
        document.querySelectorAll('.add-card').forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                const columnType = column.dataset.id === "1" ? 'pendente' :
                                column.dataset.id === "2" ? 'em-progresso' : 'finalizado';
        ...
        document.querySelectorAll('.kanban-card').forEach(cardElement => {
            const columnType = (cardElement.closest('.kanban-column') as HTMLElement).dataset.id === "1" ? 'pendente' :
                            (cardElement.closest('.kanban-column') as HTMLElement).dataset.id === "2" ? 'em-progresso' : 'finalizado';
